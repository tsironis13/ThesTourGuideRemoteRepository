...
public class TestLocalSqliteDatabase extends SQLiteOpenHelper {
	...
	private void copyDataBase() throws IOException{
	 ...
	 public Cursor getCurrentEvents(int id) throws SQLException{
		 SQLiteDatabase db = this.getReadableDatabase();
		 
		 String searchQuery = "SELECT * FROM PlacesTable WHERE _id = '" + id + "'";
		 //Cursor cursor = db.rawQuery("SELECT * FROM PlacesTable WHERE _id = ? ", new Integer[]{id});
		 Cursor cursor = db.rawQuery(searchQuery, null);
		 return cursor;
	 }
	 ...
	 public ArrayList<PlacesData> getEventsOnCalendarClick(String genre, String date) throws SQLException{
		 ...
		 if (cursor.moveToFirst()){
			 do{
				...
				if (strDate.compareTo(strfromDate) == 0 && strDate.compareTo(strtoDate) == 0){
